...
// Ключ читается из переменной окружения Netlify (раньше Vercel) GOOGLE_PLACES_KEY.
...
exports.handler = async () => {
  ...
  if (!key) return ok(FALLBACK);
  try {
    const [ru, en] = await Promise.all([fetchNew(key, 'ru'), fetchNew(key, 'en')]);
    if (!ru.reviews.length && !en.reviews.length) return ok(FALLBACK);
    return ok({
